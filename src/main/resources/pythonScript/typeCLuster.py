...
def color_cluster(list_3d,rowdata,dataMat,dataindex,dataSet,k=4):
    index=0
    ...
    for indx in range(datalen):
        ...
        if int(dataindex[indx])==0:
            ...
            list_3d[0][count0][4:7]=[int(dataindex[indx]),"MAXTEMP",nowtime0]
            count0+=1
            #======================
            list_3d[0][count0][0:2]=rowdata[index][0:2]
            ...
            list_3d[1][count1][4:7]=[int(dataindex[indx]),"MAXTEMP",nowtime1]
            count1+=1
            #======================
            list_3d[1][count1][0:2]=rowdata[index][0:2]
            ...
            list_3d[2][count2][4:7]=[int(dataindex[indx]),"MAXTEMP",nowtime2]
            count2+=1
        #======================
            list_3d[2][count2][0:2]=rowdata[index][0:2]
        ...
        #=====================
            ...
            list_3d[3][count3][4:7]=[int(dataindex[indx]),"MAXTEMP",nowtime3]
            count3+=1
            #======================
            list_3d[3][count3][0:2]=rowdata[index][0:2]
...
def start(area,mdate):
    ...
    try:
        start=time.time()
        # sqlDelete="delete from load_weather_cluster where DATE_FORMAT(mdate, '%Y')='"+mdate+"' and area=(select area from dbarea where dname='"+area+"' " \
        #            ") and cluster_flag='MAXTEMP'"
        ...
        sqlDict="select b.temperature,avg(a.maxload)from "+currentSchema+".load_daytrait a,"+currentSchema+".autovalue b where DATE(a.mdate)=DATE(b.time) " \
                ...
                "select  date from " \
                "(" \
                "SELECT Date(time) date,max (temperature) temperature " \
                "from "+currentSchema+".autovalue " \
                "where " \
                "to_char(time, 'yyyy') ='"+mdate+"' and " \
                "stcd = (select stcd  from "+currentSchema+".qstation  where stnm = '"+area+"' )  and temperature> " \
                "(SELECT (weather_sens-1)*10 FROM "+currentSchema+".load_weather_sens where Dname='"+area+"' and myear='"+mdate+"' and WEATHER_TYPE='MAXTEMP') " \
                "group by Date(time) " \
                "order by Date(time) " \
                ") a) " \
                ...
                "from "+currentSchema+".qstation where stnm = '"+area+"' )  and temperature> " \
                "(SELECT (weather_sens-1)*10 FROM "+currentSchema+".load_weather_sens where Dname='"+area+"' and myear='"+mdate+"' and WEATHER_TYPE='MAXTEMP') " \
                "group by Date(time) order by Date(time)) b ) " \
                "group by b.temperature order by b.temperature desc "
        start = time.time()
        dict={} #产生温度
        cur.execute(sqlDict)
        ...
        index=0
        while len(tempAndLoad)>0 and index<(len(tempAndLoad)-1):
            rowDataTmp[index][0]=tempAndLoad[index][0] #是为了将温度值赋予给列表
            #将计算后的差赋予给列表，温度是由高到低，因为是上比，所以温度高处的值减温度低处的值
            rowDataTmp[index][1]=float(tempAndLoad[index][1])-float(tempAndLoad[index+1][1])
            index+=1
        for row in rowDataTmp:
            dict[row[0]]=row[1]
        sql1="select a.area,a.mdate,max(b.temperature),a.maxload,a.aveload from "+currentSchema+".load_daytrait a,"+currentSchema+".autovalue b " \
             ...
             "having max(temperature)>(SELECT weather_sens*10 FROM "+currentSchema+".load_weather_sens where Dname='"+area+"' and myear='"+mdate+"' and weather_type='MAXTEMP' ) " \
             "ORDER BY DATE(time) ) group by a.area,a.mdate,a.maxload,a.aveload " \
             "having max(b.temperature)>(SELECT weather_sens*10 FROM "+currentSchema+".load_weather_sens where Dname='"+area+"' and myear='"+mdate+"' and weather_type='MAXTEMP') " \
...
#第五阶段 分类结果可视化
        ...
        if(len(list_3d)>0 and len(rowdata)>0 and len(dataMat)>0 and len(dataSet)>0):
                color_cluster(list_3d,rowdata,dataMat,ClustDist[:,0:1],dataSet)
